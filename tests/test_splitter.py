...
class TestCrisisCoreSplitter(unittest.TestCase):
    def setUp(self):
        ...
        # Create test content with short lines that will split into multiple
        # chunks
        with open(self.test_file, 'w', encoding='utf-8') as f:
            # Each line is ~9 bytes, with 20 byte chunks we should get multiple
            # chunks
            for i in range(10):
                f.write(f'Line {i}\n')
    ...
    def test_invalid_chunk_size(self):
        with self.assertRaises(Exception):
            splitter = CrisisCoreSplitter(
                str(self.test_file), max_chunk_size=-1)
            splitter.split_file()
